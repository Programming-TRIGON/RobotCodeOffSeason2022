...
public class AutoShootCommand extends ParallelCommandGroup {
    ...
    public AutoShootCommand() {
        super();
        canShootBtn = new Button(this::canShoot);
        addCommands(
                Commands.getPrimeShooterByLimelightCommand(),
                Commands.getPitchByLimelightCommand(),
                Commands.getTurnToLimelight0Command()
        );
        canShootBtn.whileHeld(Loader.getInstance().getLoadCommand());
        canShootBtn.whenReleased(Loader.getInstance().getLoadCommand().withTimeout(0.5));
    }
